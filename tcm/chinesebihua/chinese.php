<?php
	...
	class bihua
	{
	 	...
	 	function bihua()
	    {
	    	$this->charCnt=0;
	 		...
	 		$this->letterCnt=0;
	 		$this->elseCnt=0;
	    	$this->dataNumber="0123456789";
	    	$this->dataLetter="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
			//$this->dataSpecial="! ＂ ＃ ￥ ％ ＆ ＇ （ ） ＊ ＋ ， － ． ／ ： ； ＜ ＝ ＞ ？ ＠ ［ ＼ ］ ＾ ＿ ｀ ｛ ｜ ｝ ￣ 、 。 · ˉ ˇ ¨ 〃 々 — ～ ‖ … ‘ ’ “ ” 〔 〕 〈 〉 《 》 「 」 『 』 〖 〗 【 】 ± × ÷ ∶ ∧ ∨ ∑ ∏ ∪ ∩ ∈ ∷ √ ⊥ ∥ ∠ ⌒ ⊙ ∫ ∮ ≡ ≌ ≈ ∽ ∝ ≠ ≮ ≯ ≤ ≥ ∞ ∵ ∴ ♂ ♀ ° ′ ″ ℃ ＄ ¤ ￠ ￡ ‰ § № ☆ ★ ○ ● ◎ ◇ ◆ □ ■ △ ▲ ※ → ← ↑ ↓ 〓 〡 〢 〣 〤 〥 〦 〧 〨 〩 ㊣ ㎎ ㎏ ㎜ ㎝ ㎞ ㎡ ㏄ ㏎ ㏑ ㏒ ㏕ ︰ ￢ ￤ ℡ ㈱ ‐ ー ゛ ゜ ヽ ヾ 〆 ゝ ゞ ﹉ ﹊ ﹋ ﹌ ﹍ ﹎ ﹏ ﹐ ﹑ ﹒ ﹔ ﹕ ﹖ ﹗ ﹙ ﹚ ﹛ ﹜ ﹝ ﹞ ﹟ ﹠ ﹡ ﹢ ﹣ ﹤ ﹥ ﹦ ﹨ ﹩ ﹪ ﹫              〇 ─ ━ │ ┃ ┄ ┅ ┆ ┇ ┈ ┉ ┊ ┋ ┌ ┍ ┎ ┏ ┐ ┑ ┒ ┓ └ ┕ ┖ ┗ ┘ ┙ ┚ ┛ ├ ┝ ┞ ┟ ┠ ┡ ┢ ┣ ┤ ┥ ┦ ┧ ┨ ┩ ┪ ┫ ┬ ┭ ┮ ┯ ┰ ┱ ┲ ┳ ┴ ┵ ┶ ┷ ┸ ┹ ┺ ┻ ┼ ┽ ┾ ┿ ╀ ╁ ╂ ╃ ╄ ╅ ╆ ╇ ╈ ╉ ╊ ╋ ";
			$this->data[0][0]="一 ";
			$this->data[0][4]="乙 ";
		...
		*查询汉字笔画
		*/
	   function count_bihua($str)
	   {
	   	   $index=strstr($this->dataNumber,$str);
	   	   if (!($index===false))
	   	   {
	   	   		$this->numberCnt++;
	   	   		return 0;//数字
	   	   }
	   	   $index=strstr($this->dataLetter,$str);
	   	   if (!($index===false))
	   	   {
	   	   		$this->letterCnt++;
	   	   		return 0;//英文
	   ...
	   function query($str)
	   {
			...
			{
				$char = mb_substr($str, $i, 1,'utf-8');//截取字符串
				$char_num = $this->count_bihua($char);//获得笔画
				if($char_num != 0)
				{
					$num+=$char_num;
					$list[]=array('char'=>$char,'cnt'=>$char_num);
				}
			}
			$this->result = array('numberCnt'=>$this->numberCnt,'letterCnt'=>$this->letterCnt,'elseCnt'=>$this->elseCnt,'charCnt'=>$this->charCnt,'bihua'=>$num,'list'=>$list);

	   }
	   /*
		*按笔画排序
		*/
	   function sortBihua($array,$order)
	   {
	   		$sortArray = null;
	   		$resultStr = "";
	   		foreach ($array as $ss) 
			{
				$sortArray[$ss] = $this->count_bihua($ss);
			}
			if($order == "0")
				asort($sortArray);
			else
		...
		/*
	   	* Order the Chinese person/book names in ascending order
	   	*/
	   	function sortBihuaByPosition($array, $pos = 0){
			$res_arr = array();
	   		foreach ($array as $value){
				...
				$ch_arr = $this->mb_str_split($value);
				$cnt = 0;
				if (isset($ch_arr[$pos])){
					$ch = $ch_arr[$pos];
					$cnt = $this->count_bihua($ch);
				}
				//echo $ch."<br>";
				$res_arr[$cnt][] = $value;	
			}			
		...
		function sortBihuaFirstTwoCh($array){
			$res = array();
			$arr = $this->sortBihuaByPosition($array, 0);
						
			foreach ($arr as $key => $val){
				$arr2 = $this->sortBihuaByPosition($val, 1);
				foreach ($arr2 as $k => $v){
					if (gettype($v)=="array"){
	...
	function testch(){
		$arr = array("一草亭目科全书", "万氏秘传外科心法", "万氏秘传片玉心书", "王一", "丁甘仁医案", "一得集", "三因极一病证方论", "万病回春", "王", "万");
		//$arr = array("二", "万", "丁", "一", "三", "万");
		$bh = new bihua();
		//echo $bh->sortBihua($arr,"0");
		$ret = $bh->sortBihuaFirstTwoCh($arr);
		print_r($ret);
		echo "<br />";
